...
# ==========================================
# 0. NEW: Community Model (e.g., S3 MCA, S8 IMCA)
# ==========================================
class Community(models.Model):
    name = models.CharField(max_length=50, unique=True, help_text="e.g., S3 MCA, S8 IMCA")
...
# ==========================================
# 1. User Profile (To store Department & Community)
# ==========================================
class Profile(models.Model):
    ...
    # NEW: Link profile to a specific community
...
class Notice(models.Model):
    ...
    # NEW: Target specific communities (e.g., only S3 MCA)
    target_community = models.ForeignKey(Community, on_delete=models.SET_NULL, null=True, blank=True, help_text="Leave blank to target the whole department")
    ...
    # Tracks if the automated alerts have been sent
...
def create_notice_notification(sender, instance, created, **kwargs):
    if created:
        ...
        # ADMIN -> STUDENTS (FILTERED BY COMMUNITY OR DEPARTMENT)
        students = User.objects.filter(is_staff=False, is_superuser=False)
        
        # Check if there's a specific community targeted first
        if instance.target_community:
            students = students.filter(profile__community=instance.target_community)
            target_label = instance.target_community.name
        # If no community, fall back to checking the department
        elif instance.target_department != 'All':
            students = students.filter(profile__department=instance.target_department)
            target_label = instance.target_department
        else:
        ...
        if instance.expires_at:
            ...
            email_deadline = instance.expires_at.strftime('%A, %d %B %Y at %I:%M %p') 
        
        # 1. CREATE IN-APP NOTIFICATIONS
        notifications = [
            Notification(
                recipient=student,
                message=f"[{target_label}] New Notice: {instance.title}{deadline_info}",
                related_notice=instance
        ...
        if student_emails:
            ...
            subject = f"New Campus Notice [{target_label}]: {instance.title}"
            
            text_body = f"Hello,\n\nA new notice has been posted for {target_label}.\n\nTITLE: {instance.title}\nDEADLINE: {email_deadline}\n\nPlease view and acknowledge it here: {notice_link}"
            ...
            <html>
              <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6;">
                ...
                <p>Hello,</p>
                <p>A new notice has been posted for <strong>{target_label}</strong>.</p>
                <p>
...
class Poll(models.Model):
    ...
    # NEW: Target specific communities
    target_community = models.ForeignKey(Community, on_delete=models.SET_NULL, null=True, blank=True, help_text="Leave blank to target the whole department")
...
def create_poll_notification(sender, instance, created, **kwargs):
    if created:
        ...
        # Filter by community first, then department
        if instance.target_community:
            students = students.filter(profile__community=instance.target_community)
        elif instance.target_department != 'All':
            students = students.filter(profile__department=instance.target_department)
