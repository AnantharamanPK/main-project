...
# --- NEW: Imported Community ---
from .models import Notice, User, NoticeReadStatus, DirectMessage, Notification, Poll, PollResponse, Community 
from .forms import NewNoticeForm

# 1. MAIN LIST VIEW (Board filtered by Department & Community)
class NoticeListView(LoginRequiredMixin, ListView): 
    ...
    def get_queryset(self):
        ...
        # DEPARTMENT & COMMUNITY FILTERING
        if user.is_authenticated and not user.is_staff:
            try:
                user_dept = user.profile.department
                user_community = getattr(user.profile, 'community', None)
                
                # Must match Department (or All) AND Community (or Blank)
                queryset = queryset.filter(
                    Q(target_department=user_dept) | Q(target_department='All')
                ).filter(
                    Q(target_community=user_community) | Q(target_community__isnull=True)
                )
            except:
                # Fallback if profile is missing
                queryset = queryset.filter(target_department='All', target_community__isnull=True)
                
            # --- NEW: TEMPORAL FILTER (Students only see notices created AFTER they joined) ---
            queryset = queryset.filter(created_at__gte=user.date_joined)
            # ---------------------------------------------------------------------------------
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated:
            ...
            # ==========================================
            # NEW: POLL LOGIC (3-Way Check with Communities)
            # ==========================================
            try:
                user_dept = user.profile.department
                user_community = getattr(user.profile, 'community', None)
            ...
            # 1. Get active polls for this user's department/community (and 'All')
            if user.is_superuser:
                # Admins see all active polls to monitor them
                active_polls = Poll.objects.filter(is_active=True)
            elif user.is_staff:
                # Staff see NOTHING (Polls are hidden from teachers/faculty)
                active_polls = Poll.objects.none()
            else:
                # Students see targeted polls created after they joined
                active_polls = Poll.objects.filter(
                    is_active=True, 
                    created_at__gte=user.date_joined  # <--- NEW: TEMPORAL FILTER FOR POLLS
                ).filter(
                    Q(target_department=user_dept) | Q(target_department='All')
                ).filter(
                    Q(target_community=user_community) | Q(target_community__isnull=True)
...
# 2. ARCHIVE VIEW (Department & Community Aware)
class ArchivedNoticeListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated and not user.is_staff:
            ...
            # --- NEW: TEMPORAL FILTER FOR ARCHIVES ---
            queryset = queryset.filter(created_at__gte=user.date_joined)
            # -----------------------------------------
...
def NoticeView(request, notice_id):
    ...
    # Added Check for Community!
    if notice.target_community:
        target_students = User.objects.filter(
            profile__community=notice.target_community, 
            is_staff=False, 
            is_superuser=False
        )
    elif notice.target_department == 'All':
...
# 11. MARK ALL AS READ (Filtered by Department & Community)
@login_required
def mark_all_as_read(request):
    if request.method == 'POST':
        ...
        all_eligible_notices = Notice.objects.filter(
            is_approved=True,
            created_at__gte=user.date_joined  # <--- NEW: TEMPORAL FILTER ADDED HERE
        ).filter(
            Q(target_department=user_dept) | Q(target_department='All')
        ).filter(
            Q(target_community=user_community) | Q(target_community__isnull=True)
...
def analytics_dashboard(request):
    ...
    # 3. Department Read Engagement (Added IMCA)
    departments = ['MCA', 'BTech', 'MBA', 'IMCA']
    dept_stats = []
...
def get_calendar_events(request):
    ...
    # Determine department and community
    if user.is_staff or user.is_superuser:
    ...
    if user.is_superuser:
        ...
        # Staff get an empty list of polls for their calendar
...
# ==========================================
# 17. AJAX ENDPOINT FOR COMMUNITIES (NEW)
# ==========================================
def load_communities(request):
    """
    AJAX endpoint that returns a JSON list of communities 
    filtered by the selected department.
    """
    department = request.GET.get('department')
    
    if department == 'All' or not department:
        # If 'All' is selected, return all communities
        communities = Community.objects.all().order_by('name')
    else:
        # Otherwise, strictly filter by the selected department
        communities = Community.objects.filter(department=department).order_by('name')
    
    # Convert the queryset into a list of dictionaries
    # We only need the ID and the Name for the dropdown
    community_list = list(communities.values('id', 'name'))
